  ...
  <form class="flex max-w-full justify-center" @submit.prevent="research">
    ...
    <div :id="id" class="over-y z-10 hidden bg-gray-50 divide-y divide-gray-100 shadow-lg w-36 h-[170px] dark:bg-gray-700">
      <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdown-button">
        ...
        <li v-for="category in categories" :key="category.id" class="border-b my-border-gray">
          <span class="block px-4 py-2 hover:text-white hover:bg-sheet-200 dark:hover:bg-gray-600 dark:hover:text-white" @click="filterForm.category = category.id">
            {{ category.name }}
          </span>
        </li>
      </ul>
    ...
    <div class="relative w-full">
      <input id="search-dropdown" v-model="filterForm.q" type="search" class="bg-mango block p-4 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-r-md border-l-gray-100 border-l-2 border border-gray-300 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:border-green-500" placeholder="Search Product here ..." required />
      <button type="submit" class="absolute top-0 right-0 p-4 text-sm font-medium text-white bg-green-700 rounded-r-md border border-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"><svg aria-hidden="true" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg></button>
    </div>
  </form>
</template>
