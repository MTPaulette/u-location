<template>
  ...
  <div v-if="post.images.length" class="relative h-3/5 w-full">
    ...
    <!-- <Link :href="route('dashboard.post.image.destroy', {post: props.post.id, image: post.images[selectedImage].id})" method="DELETE" as="btn" class="absolute bg-mango text-white hover:text-danger top-0 right-0 p-2">
    ...
    <ul class="flex flex-wrap gap-4 justify-center items-center my-12">
      <li v-for="(image, i) in post.images" :key="image.id" class="relative" @click="selectedImage=i">
        <Link :href="route('dashboard.post.image.destroy', {post: post.id, image: image.id})" method="DELETE" as="btn" class="absolute text-white hover:text-danger top-0 right-0 pt-1 pl-1" :class="selectedImage==i?'bg-mango': 'bg-gray-700'">
