  ...
  >
    ...
    <div v-if="product.images.length > 0" class="relative h-3/5 w-full">
      <div class="border-2 border-mango rounded">
        <!-- <Link :href="route('product.image.destroy', {product: props.product.id, image: product.images[selectedImage].id})" method="DELETE" as="btn" class="absolute bg-mango text-white hover:text-red-500 top-0 right-0 p-2">
      ...
      <ul class="flex flex-wrap gap-4 justify-center items-center my-12">
        <li v-for="(image, i) in product.images" :key="image.id" class="relative" @click="selectedImage=i">
          <Link :href="route('product.image.destroy', {product: product.id, image: image.id})" method="DELETE" as="btn" class="absolute text-white hover:text-red-500 top-0 right-0 pt-1 pl-1" :class="selectedImage==i?'bg-mango': 'bg-gray-700'">
