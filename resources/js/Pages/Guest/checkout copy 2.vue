        ...
        <div class="col-span-3 order-2 md:order-1">
          ...
          <div class="mt-4 md:mt-0 md:col-span-3 lg:col-span-2 w-full px-5 py-4 h-auto bg-white border my-border-gray rounded-lg dark:bg-gray-800">
              ...
              <form method="PUT" class="flex flex-col justify-between w-full h-auto" @submit.prevent="updateUserInformation">
                <div v-if="!user" class="grid gap-2 sm:gap-6 grid-cols-2">
                  ...
                  <div>
                    ...
                    <input id="email" v-model="formUser.email" type="email" class="input disabled:bg-blue-50 dark:disabled:bg-gray-900 dark:disabled:opacity-50" />
                ...
                <div class="w-full block sm:flex justify-between items-end gap-2 mt-2 sm:mt-6">
                  <div class="w-full">
                    ...
                    <select id="address" v-model="formUser.address_id" name="weight" class="input">
                ...
                <div class="w-full flex items-start mt-2 sm:mt-6">
                  <div class="flex items-center h-5">
                    <input id="paiement" v-model="formUser.paiement_mode" type="checkbox" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-mango dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-mango dark:ring-offset-gray-800" />
                  </div>
            ...
            <div v-else>
              ...
              <form method="POST" class="flex flex-col justify-between w-full h-auto" @submit.prevent="saveNewAddress()">
                  ...
                  <div>
                    ...
                    <select id="city" v-model="formUser.city_id" name="weight" class="input">
                      <optgroup v-for="cityGroup in cities" :key="cityGroup" :value="cityGroup">
                        <option v-for="city in cityGroup" :key="city.id" :value="city.id">{{ city.name }}</option>
                      </optgroup>
                    </select>
                  </div>
                  <div class="col-span-2">
                    <label for="address" class="label">Address</label>
                    <input id="address" v-model="formUser.street" type="text" class="input" placeholder="eg: Carrefour rond point express" />
                    <p class="input-error">{{ formUser.errors.street }}</p>
                  </div>
                </div>
                ...
                <div class="w-full flex items-start mt-2 sm:mt-6">
                  <div class="flex items-center h-5">
                    <input id="paiement" type="checkbox" value="" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-mango dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-mango dark:ring-offset-gray-800" />
                  </div>
                  <label for="paiement" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-200">confirmer le paiement a la livraison.</label>
                </div>
...
const showAddressFrom = () => {
  ...
  if(cartcount.value) {
    formUser.new_address = true
  }
}

const checkout = () => formUser.post('/checkout')
</script>
